...
$(document).ready(function() {
    ...
    function fetchRecords() {
        fetch('/hr-payroll-master/application/views/backend/fetch.php')
            .then(response => response.json())
            .then(data => {
                const tableBody = document.getElementById('userRecords');
                tableBody.innerHTML = '';
                ...
                data.forEach(record => {
                    ...
                    row.innerHTML = `
                        ...
                        <td>${record.time_out || 'No Time Out'}</td>
                        <td>${record.time_pairs || ''}</td>
                        <td>${record.late || '00:00'}</td>
                        <td>${record.undertime || '00:00'}</td>
                        <td>${record.absent ? 'Absent' : 'Present'}</td>
                        <td>${record.hours_worked || '00:00'}</td>
                        <td>${record.prod_hrs || '00:00'}</td>
    ...
    fetchRecords(); // Call fetchRecords after document is ready

    window.editRecord = function(id) {
        window.location.href = `/hr-payroll-master/application/views/backend/edit.php?id=${id}`;
    }

    window.deleteRecord = function(id) {
        if (confirm('Are you sure you want to delete this record?')) {
            fetch(`/hr-payroll-master/application/views/backend/delete.php?id=${id}`, { method: 'DELETE' })
